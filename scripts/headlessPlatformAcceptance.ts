...
try {
  ...
  await assertCandidateAsync(consumerRoot, candidate.name, candidate.version, candidate.filename);
...
async function assertCandidateAsync(
  consumerRoot: string,
  ...
  const manifest = await readJsonFileAsync<{ name?: string; version?: string }>(
    path.join(consumerRoot, 'node_modules', candidateName, 'package.json'),
  );
  if (manifest.name !== candidateName || manifest.version !== candidateVersion) {
    throw new Error('Installed candidate metadata does not match the packed artifact.');
  }
  const lockfile = await Bun.file(path.join(consumerRoot, 'bun.lock')).text();
  if (!lockfile.includes(candidateFilename)) {
