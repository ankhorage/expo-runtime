...
- **CHANGE REQUIRED — Expo 57 patch baseline.** The exact runtime baseline is Expo 57.0.18 with Expo Doctor 1.20.4. The full owned compatibility projection was refreshed from that release's bundled native module map, including its current Router, Metro runtime, Asset, Auth Session, Constants, Dev Client, File System, Font, Image Picker, Linking, Location, Notifications, Secure Store, Splash Screen, and Updates ranges. The clean fixture compares every projected range with the installed Expo package before running Expo's compatibility checks, so a lower compatible range cannot hide stale package metadata.
